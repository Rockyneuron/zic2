function[SNR]=sig2noise_2_2(It,im,roiS,roiB)
%--------------------------------------------------------------------------
...
[SNR.glb_rat,SNR.glb_c]=SNRrat_1(totr,It.Gp0,roiS);
...
errorbar(1:20,SNR.glb_rat(1,:),SNR.glb_rat(2,:),'-o','LineWidth',wd ),hold on,
...
errorbar(1:20,SNR.glb_c(1,:),SNR.glb_c(2,:),'-o','LineWidth',wd ),hold on,
line(1:length(SNR.blresp),repmat(0,1,length(SNR.blresp)),'Color','Black','LineWidth',wd )
...
f4.Name='SNR contrat';
    ...
    function[SNRrat,SNRc]=SNRrat_1_1(sig,back,stb)
    
        SNR=((sig-back)./repmat(stb,[size(sig,1),1]));
      SNRrat(1,:)=nanmean(SNR);
        SNRrat(2,:)=nanstd(SNR);

       

SNR=(sig-back)./(sig+back);
 SNRc(1,:)=nanmean(SNR);
        SNRc(2,:)=nanstd(SNR);
...
[SNR,SNRc]=SNRrat_1_1(signal,background,stb);
